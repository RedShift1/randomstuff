...
FILENAME=$(hostname).$(date +"%Y-%m-%d").level${LEVEL}.$(date +"%a").dump.gz

/sbin/dump -${LEVEL} -uanL -b128 -C8 -f - / | /usr/bin/gzip -2 | \
 /usr/bin/ssh -T -x -o Compression=no -c arcfour ${DEST} "dd of=${FILENAME}.tmp bs=1M; "'MD5=$(md5sum '"${FILENAME}.tmp"' | cut -d " " -f 1);'"mv ${FILENAME}.tmp ${FILENAME}."'${MD5}'
